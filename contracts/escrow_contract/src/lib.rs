...
let (_, _, _, escrow_id) =
    setup_disputed_escrow(&env, &client, Some(unregistered_arbiter.clone()));
        ...
        fn test_registered_arbiter_can_resolve_dispute_and_events() {
            ...
            let events = env.events().all();
Here is the resolved Rust code with all merge conflicts resolved:
...
```
        ...
        fn test_remove_arbiter() {
            ...
            let events = env.events().all();
Here is the resolved line of code:
...
```
            let found_removed_event = events.iter().any(|(_, topics, _)| {
                topics
                    .get(0)
                    .and_then(|v| Symbol::try_from_val(&env, &v).ok())
Here is the resolved line of code using the `target_rem` symbol defined above:
